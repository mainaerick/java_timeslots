...
class logic {
    public static void main(String[] args) throws ParseException {
        String[] time = { "2014-11-25 07:00", "2014-11-25 11:00", "2014-11-25 14:00", "2014-11-25 16:30" };
        String[] totime = { "2014-11-25 09:00", "2014-11-25 13:00", "2014-11-25 16:00", "2014-11-25 18:00" };
        ...
        Date first_task_date=format.parse(time[0]);
        ...
        Date first_task_time = format.parse(time[0]);
        cal2.setTime(first_task_time);
         

        // if day start ie 7:00 am is less than first task time 8:00, have a free time slot of one hour from 7-8
        if(cal.before(cal2)){
        ...
        while (cal.getTime().before(end_time_day)) { // loop through every minute form day start time to day end time
        ...
        for (int i = 0; i < time.length; i++) {
            ...
            Date date2 = format.parse(totime[i]);
            ...
            for (int j = 0; j < intervals.size(); j++) {
                ...
                if (format2.format(intervals.get(j)).equals(totime_string)) {
                    ...
                    if (i + 1 == time.length) {
                        ...
                        date = format.parse(time[i + 1]); // else set date to next task start time
                    }
            ...
            try {
                ...
                LocalDateTime lDateTime1 = LocalDateTime.parse(time[i], formatter); // from time localdatetime
                LocalDateTime lDateTime2 = LocalDateTime.parse(totime[i], formatter); // to time local date time
